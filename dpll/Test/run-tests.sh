...
for entree in `ls Test/ | sort -n`; do
...
# et stocke cette valeur dans le fichier /tmp/toto.txt
  echo $entree | /usr/bin/time -f'%U' -o /tmp/toto.txt ./dpll "Test/$entree"
# on recupere ensuite dans TEMPS1 le contenu de /tmp/toto.txt
# notez a nouveau l'usage des `..`
  TEMPS1=`cat /tmp/toto.txt`

  echo $entree | /usr/bin/time -f'%U' -o /tmp/toto.txt ./dpll "Test/$entree" -rand
  TEMPS2=`cat /tmp/toto.txt`
  
   echo $entree | /usr/bin/time -f'%U' -o /tmp/toto.txt ./dpll "Test/$entree" -moms
  TEMPS3=`cat /tmp/toto.txt`
  
   echo $entree | /usr/bin/time -f'%U' -o /tmp/toto.txt ./dpll "Test/$entree" -dlis
