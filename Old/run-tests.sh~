...
echo "argument default default_wlit rand rand_wlit" >> comparaison.dat
...
# et stocke cette valeur dans le fichier /tmp/toto.txt
  echo $entree | /usr/bin/time -f'%U' -o /tmp/toto.txt ./dpll "ufTest/$entree.cnf"
# on recupere ensuite dans TEMPS1 le contenu de /tmp/toto.txt
# notez a nouveau l'usage des `..`
  TEMPS1=`cat /tmp/toto.txt`

  echo $entree | /usr/bin/time -f'%U' -o /tmp/toto.txt ./dpll "ufTest/$entree.cnf" -wlit
  TEMPS2=`cat /tmp/toto.txt`

  echo $entree | /usr/bin/time -f'%U' -o /tmp/toto.txt ./dpll "ufTest/$entree.cnf" -rand
  TEMPS3=`cat /tmp/toto.txt`
  
  echo $entree | /usr/bin/time -f'%U' -o /tmp/toto.txt ./dpll "ufTest/$entree.cnf" -wlit -rand
  TEMPS4=`cat /tmp/toto.txt`

# pour finir, on ajoute une ligne (3 colonnes) au fichier ./comparaison.dat
 echo $entree $TEMPS1 $TEMPS2 $TEMPS3 $TEMPS4 >> ./comparaison.dat
# echo $entree $TEMPS1 $TEMPS3 >> ./comparaison.dat
